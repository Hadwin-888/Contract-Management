...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  try {
    await prisma.procurementRequest.delete({ where: { id: req.params.id as string as string } });
...
router.post('/:id/submit', async (req: AuthRequest, res: Response) => {
  try {
    const request = await prisma.procurementRequest.findUnique({ where: { id: req.params.id as string as string } });
    if (!request) {
      res.status(404).json({ error: '采购申请不存在' });
      return;
    }
        ...
        for (const userRole of role.userRoles) {
          await prisma.approvalRecord.create({
            data: {
              flowId: flow.id,
              stepId: step.id,
              requestId: req.params.id as string as string,
          ...
          await prisma.notification.create({
            data: {
              ...
              module: 'procurement',
              refId: req.params.id as string as string,
            },
          });
        }
      }
